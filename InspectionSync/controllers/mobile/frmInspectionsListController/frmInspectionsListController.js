define({ 
  ...
  onNavigate:function(param){
    ...
    if(kony.sdk.isNullOrUndefined(param)){
      ...
      if(!kony.sdk.isNullOrUndefined(param.previousForm) && param.previousForm=="frmInspectionReview"){
        this._previousForm = param.previousForm;
        return;
      }
    }
  ...
  processRecords:function(){
    ...
    var assetRepresenterMap=InspectionUtil.parseRecords(this.assetRepresenterList ,"Id");
      ...
      for(var i=0;i<this.assetsList.length;i++){
        ...
        if(typeof asset==='object' && asset!==null ){
          ...
          if(typeof asset["Asset_Type_Id"]==='string' && typeof assetTypeListMap==='object' && assetTypeListMap!==null){
            if(Array.isArray(assetTypeListMap[asset["Asset_Type_Id"]]) && (assetTypeListMap[asset["Asset_Type_Id"]]).length>0){
              asset["type"]=assetTypeListMap[asset["Asset_Type_Id"]][0];
            }
          }
          if((typeof asset["Representer_Id"]==='string' || typeof asset["Representer_Id"]==='number') && 
             typeof assetRepresenterMap ==='object' && assetRepresenterMap!==null){
            if(Array.isArray(assetRepresenterMap[asset["Representer_Id"]]) && (assetRepresenterMap[asset["Representer_Id"]]).length>0){
              asset["representer"]=assetRepresenterMap[asset["Representer_Id"]][0];
            }
          }
        }
      }
  ...
  onFormPostShow:function(){
    debugger;
    if(!kony.sdk.isNullOrUndefined(this._previousForm) && this._previousForm=="frmInspectionReview"){
      return;
    }
    var self=this;
    try{
   ...
   */
  getLocalReadableAssignedTimestamp:function(assignedTimestamp){
    var localReadableDateTimeString=null;
    if(typeof assignedTimestamp==='string'){
      try{
        var jsDateObj=InspectionUtil.getSqlDatetoJSDate(assignedTimestamp);
        //var localDateTimeObj=InspectionUtil.getUTCtoLocalDatetime(jsDateObj);
        localReadableDateTimeString=InspectionUtil.getReadableTimeString(jsDateObj);
        localReadableDateTimeString=localReadableDateTimeString+", "+InspectionUtil.getReadableDateString(jsDateObj);
      }catch(excp){
        debugger;
        localReadableDateTimeString=null;
        throw excp;
      }
    }else{
      debugger;
    }
    return localReadableDateTimeString;
  },
  /**
   * @function
   *
    ...
    try{
      if(Array.isArray(records)){
        ...
        for(var i=0;i<recordLength;i++){
          ...
          switch(status){
            case "assigned":
              segObj["lblInspectionStatus"]={skin:"sknLblAssigned",text:"Scheduled"};
              segObj["imgStatusInspection"]="back_imagebottom.png";
              break;
            case "in progress":
              segObj["lblInspectionStatus"]={skin:"skinLblInProgress",text:"In-progress"};
              segObj["imgStatusInspection"]="inprogress_icon.png";
              break;
            case "completed":
              //segObj["lblInspectionStatus"]="Completed";
              segObj["lblInspectionStatus"]={skin:"sknLblcompleted",text:"Completed"};
              segObj["imgStatusInspection"]="complete_icon.png";
          }
          segObj["lblAssetId"]=app_constant.asset+inspection["Asset_Id"];
          segObj["lblTime"]=this.getLocalReadableAssignedTimestamp(inspection["Assigned_Timestamp"]);
          segObj["imgMapIcon"]={"src":"map_icon.png"};
          segObj["lblSep"]=" ";
          var currLat = kony.store.getItem("currLat");
          var currLong  = kony.store.getItem("currLong");
          var desLat = Number(inspection.asset.location.Latitude);
          var desLong  = Number(inspection.asset.location.Longitude)
        ...
        this.view.SegInspectionDetails.setData(segList);
        // this.view.SegInspectionDetails.addAll([[segHeader,segList]]);
        this.view.lblTotalTask.text =recordLength+" Results";;
      }
    }catch(excp){
      debugger;
    }
  ...
  getInspectionListData:function(){
    ...
    this._fetchRecords(DATA_MODEL.ASSET);
    this._fetchRecords(DATA_MODEL.ASSET_LOCATION);
    this._fetchRecords(DATA_MODEL.ASSET_TYPE);
    this._fetchRecords(DATA_MODEL.ASSET_REPRESENTER);
    ...
    function successCB(record){
      ...
      switch(dataModel){
        ...
        case "asset":
          ...
          break;
        case DATA_MODEL.ASSET_REPRESENTER:
          this.assetRepresenterList =record;
          break;
        case "asset_type":
  ...
  populateInspectionsToSegment2:function(records){
    ...
    this.view.SegInspectionDetails.removeAll();
    this.view.SegInspectionDetails.addAll(segList);
    // this.view.SegInspectionDetails.addAll([[segHeader,segList]]);
  },
        ...
        if(typeof inspectionId ==='string' || typeof inspectionId ==='number'){
          ...
          if(typeof inspection==='object' && inspection!==null){
            ...
            if(status==="completed"){
              ...
              obj={};
              obj["previousForm"]="frmInspectionsList";
              obj["status"]="completed";
              obj["inspection"]=inspection;
              obj["userData"] = this._navigationData;
              try{
  ...
  _fetchDataSuccess: function(inspectionID,response){
    if(response.records!==undefined && response.records[0]!==undefined){
      ...
      param["previousForm"]="frmInspectionsList";
      try{
        var navObj=new kony.mvc.Navigation("frmInspectionReview");
  ...
  populateFilterCount: function(){
    if(this._filterCount>0){
      ...
      //this.view.lblFilter.skin = "sknLblInspectionFIlter666eff";sknLblInspectionFIlter
  ...
  changeFocusSkin:function(widgetId){
    ...
    this.view.flxDashBoard.skin = "sknFlxHamburgerUnselected";
    ...
    try{
      switch(widgetId){
        ...
        case "flxAddInspection":
          ...
          break;
        case "flxDashBoard":
          this.view.flxDashBoard.skin = "sknFlxHamburgerSelected";
          this.changeFlxAccountProperties();
          this.changeSelectedFlexSkin("flxDashBoard");
          break;
          case "flxSync":
  ...
  setHamburgerData: function(){
    var data = this._navigationData;
  ...
  onSortFlexClick:function(eventobject){
    if(eventobject!==null && eventobject!==undefined){
      if(eventobject.id==="flxNearestToMe"){
        this.view.imgRadiobtn1.src="radio_selected.png";
        this.view.imgRadioBtn2.src="radio_unselected.png";

        this.isSortByDate=false;
        this.isSortByDistance=true;

      }else if(eventobject.id==="flxMostRecentInspection"){
        this.view.imgRadiobtn1.src="radio_unselected.png";
